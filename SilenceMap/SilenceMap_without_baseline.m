...
clc
%%
% SilenceMap without baseline algorithm test code
...
for ref_ind = 1:10
    ...
    % figure;
    % h1 = plot_source_space_signal_vIV(Betta,sulc,cortex1,cortex1);
    % axis off
    % axis equal
    ...
    for i=1:size(k_search_grid,2)
        k = k_search_grid(i);
        Sigma_s = ones(p,1);
        x = CSpeC(L1,cortex1,Betta,[],[],k,[],pow_flag,[]);% CSpeC function
        x_tot = cat(2,x_tot,x);
        [~,idx_x] = sort(x,'ascend');
        Sigma_s(idx_x(1:k)) = 0;
...
%     k = 10;
    ...
    while((sum((silence_center_old-silence_center).^2,2)>100 ||...
        ...
        for i=1:size(k_search_grid,2)
            k = k_search_grid(i);
            x = CSpeC(L1,cortex1,Betta,Cs,P_M,k,Epsil,pow_flag,elec_index);% CSpeC function
            x_tot = cat(2,x_tot,x);
            [~,idx_x] = sort(x,'ascend');
            Cs_silence = Cs;
...
%         k = 50;
        ...
        Result_file = sprintf('Noisy_OT_woB_kk_%d_%d_indx_center_%d_i_ref_%d.mat', kk, repp, indx_center, i_ref)
